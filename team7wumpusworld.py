...
def place_pits():
    empty_cells = [(i, j) for i in range(1, 5) for j in range(1, 5) if not (i == 1 and j == 1) and not (i == 2 and j == 1) and not (i == 1 and j == 2)]
    random.shuffle(empty_cells)  # 랜덤한 순서로 셀을 선택하기 위해 empty_cells를 섞습니다.

    for cell in empty_cells:
        if grid[cell[0]][cell[1]] != 'gold' and grid[cell[0]][cell[1]] != 'stench':
            grid[cell[0]][cell[1]] = 'pit'
            
            # 주변 칸에 stench 값을 설정합니다.
...
def goForward():
    ...
    next_position = [agent[0], agent[1]]  # 현재 위치로 초기화
...
def auto_play():
    ...
    while True:
        ...
        if grid[agent[0]][agent[1]] == 'gold':
            ...
            print("breeze!!")
            grid1[agent[0]][agent[1]] = 'breeze'  # Update grid1 with breeze
            goForward()
            stack.append(agent[:])
            continue
            ...
            print("stench!!")
            grid1[agent[0]][agent[1]] = 'stench'  # Update grid1 with stench
            goForward()
            stack.append(agent[:])
            continue
            ...
            grid1[agent[0]][agent[1]] = 'wumpus'  # Update grid1 with wumpus
            agent = [1,1]
            direction = 'East'
            stack.clear()
            continue
            ...
            print("Dead by pit")
            pit_position = agent[:]
            grid1[agent[0]][agent[1]] = 'pit'  # Update grid1 with pit
            agent = [1,1]
            direction = 'East'
            stack.clear()
            continue
